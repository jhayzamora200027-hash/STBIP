...
@section('content')
<!-- Swiper and Chart.js dependencies for gallery slider -->
...
document.addEventListener("DOMContentLoaded", function () {
	...
	const modalViewport = document.getElementById('sliderModalViewport');
...
// modal image gesture/zoom handlers removed — province helpers preserved
	...
	function openRegionStatsModalForImg(img){
		try {
			...
			const modal = document.getElementById('regionStatsModal');
			if (!modal) return;
			modal.setAttribute('aria-hidden','false');
			modal.style.display = 'block';
			document.body.classList.add('rsm-open');

			const titleEl = document.getElementById('regionStatsModalTitle');
...
// show modal image immediately (animation removed)
            try {
                const modalImgEl = document.getElementById('rsm-modal-image');
                const srcFull = src || img.src || '';
                if (modalImgEl) {
                    modalImgEl.src = srcFull;
                    modalImgEl.style.visibility = 'visible';
                }
            } catch(e) { console.error('rsm animation removed', e); try { const ri = document.getElementById('rsm-modal-image'); if (ri) { ri.src = src || ''; ri.style.visibility = 'visible'; } } catch(e){} }


			// show loader, hide content while fetching
			document.getElementById('rsm-loading').style.display = '';
		...
		const grouped = normalizeGrouping(rawGrouped);
					...
					document.getElementById('rsm-total-sts').textContent = String(allRows.length || 0);
				try { const modalTotal = document.getElementById('modalStatsTotal'); if (modalTotal) modalTotal.textContent = `Total STs: ${allRows.length || 0}`; } catch(e) {}
					document.getElementById('rsm-total-moa').textContent = String(allRows.reduce((acc,r) => acc + (truthy(r.with_moa) ? 1 : 0), 0));
				document.getElementById('rsm-total-expr').textContent = String(allRows.reduce((acc,r) => acc + (truthy(r.with_expr) ? 1 : 0), 0));
					...
					document.getElementById('rsm-total-moa-attachments').textContent = String(moaAttachments || 0);
				...
				try {
					...
					document.getElementById('rsm-cards').style.display = '';

					// load the ST listing (HTML partial) for the region
                (function renderRegionAggregatedTitles(){
                    try {
                        const stListEl = document.getElementById('rsm-st-list');
                        if (!stListEl) return;
				...
				.catch(err => {
					document.getElementById('rsm-loading').style.display = 'none';
					document.getElementById('rsm-cards').style.display = 'none';
					document.getElementById('rsm-provinces').innerHTML = '<div class="rsm-empty">Failed to load provinces</div>';
					document.getElementById('rsm-st-list').innerHTML = '<div class="rsm-empty">Failed to load STs listing</div>';
					console.error(err);
				});
...
<div class="slider-modal" id="sliderModal">
    ...
    <div class="slider-modal-content" id="sliderModalContent" style="position:relative; flex-direction:column; align-items:center; justify-content:flex-start;">
        ...
        <!-- Province listing card (populated from STsReport data via AJAX) -->
        <!--
        <aside id="sliderProvinceListCard" class="slider-province-card" aria-hidden="true">
            <div class="province-card-title">Provinces</div>
            <div id="sliderProvinceList" class="province-list">
                <div class="province-empty">No provinces yet</div>
            </div>
        ...
        <!-- viewport wrapper: used to apply inverse-scaling so modal preview is fixed across browser zoom -->
        ...
        <!-- visible wrapper placed next to the image (panel) -->
        <div class="slider-province-wrap" id="sliderProvinceWrap" role="complementary" aria-label="Region panel" aria-hidden="false">
            ...
            <div class="province-panel-body">



                <!-- Total STs card (moved inside right-side panel) -->
                ...
                <!-- Total Expression of Interest (slider panel) -->
                <div id="sliderProvinceExprCard" class="slider-province-total-card" aria-hidden="true" role="status" aria-label="Region total Expression of Interest">
                    <div class="region-st-title">Total Expression of Interest</div>
...
.slider-wrapper { position: relative; display: flex; flex-direction: column; align-items: center; width: 100%; }
/* shift slider slightly left so the active (center) card appears visually centered with the left preview */
.swiper { width: 100vw; height: 75vh; min-height: 350px; transform: translateX(-180px); transition: transform 320ms cubic-bezier(.2,.9,.2,1); }
.swiper-slide { width: 85vw; max-width: 420px; height: 65vh; min-height: 400px; border-radius: 20px; overflow: hidden; transition: 0.4s ease; display: flex; justify-content: center; align-items: center; cursor: pointer; }
.swiper-slide img { width: 100%; height: 100%; object-fit: contain; border-radius: 18px; }
.swiper-slide-active { transform: scale(1.25); }
.swiper-button-next, .swiper-button-prev { color: #0d47a1; }
@media (max-width:1200px) { .swiper { transform: translateX(-120px); } }
...
n    /* make cards flow horizontally (override absolute positioning used on large screens) */
    #stTitlesPanel, #sliderAllTitleListCard, #sliderMoaCard, #sliderUploadedCard, #sliderExprCard, #sliderResCard, #sliderTotalStCard, #sliderStatsCard, #sliderProvinceWrap {
...
n    /* smooth transitions for card visibility and layout changes */
    #stTitlesPanel, #sliderAllTitleListCard, #sliderMoaCard, #sliderUploadedCard, #sliderExprCard, #sliderResCard, #sliderTotalStCard, #sliderStatsCard, #sliderProvinceWrap { transition: all 260ms cubic-bezier(.2,.9,.2,1); }
}

/* Close button removed: modal closing still works via overlay / Escape key */
...
.rsm-provinces-list .province-sublist .city-item {
  ...
  width: calc(100% - 6px) !important; /* account for 3px border on left+right */
  max-width: calc(100% - 6px) !important;
  overflow: hidden !important;
}

/* Keep internal padding but prevent visual overflow from shadows/borders */
.rsm-provinces-list .province-item > *,
.slider-province-card .province-item > * {
  box-sizing: border-box !important;
}

/* If container still clips due to border-radius, ensure parent hides overflow */
.rsm-provinces-list, .slider-province-card { overflow: hidden; }
...
.slider-province-card .province-sublist .city-item:hover { background: #f8fafc; }
...
.rsm-provinces-list .province-sublist .st-list > div {
  max-height: 220px; /* clamp height so inner list can scroll */
  overflow: auto;
  -webkit-overflow-scrolling: touch;
}
.rsm-provinces-list .province-sublist .st-list > div::-webkit-scrollbar { width: 8px; }
.rsm-provinces-list .province-sublist .st-list > div::-webkit-scrollbar-thumb { background: rgba(2,6,23,0.06); border-radius:6px; }

/* Region Stats modal: province/city expansion styles (same as slider, scoped for rsm) */
.rsm-provinces-list .province-sublist { margin-top:6px; padding:6px 6px 8px 8px; border-left: 2px solid rgba(2,6,23,0.04); background: linear-gradient(180deg, rgba(15,23,36,0.02), transparent); border-radius:8px; overflow:hidden; }
.rsm-provinces-list .province-sublist .city-item { padding:6px 8px; border-radius:6px; display:flex; justify-content:space-between; gap:8px; align-items:center; cursor:pointer; border-bottom:1px solid rgba(2,6,23,0.04); }
.rsm-provinces-list .province-sublist .city-item:hover { background: #f8fafc; }
.rsm-provinces-list .province-sublist .province-empty { color:#6b7280; padding:8px 6px; }
.rsm-provinces-list .province-sublist .city-item .province-badge { background:#f1f5f9; padding:4px 8px; border-radius:999px; font-size:0.8rem; color:#0f1724; border:1px solid rgba(2,6,23,0.04); }

/* back controls */
.slider-province-card .prov-collapse-back, .slider-province-card .city-back-btn { background:transparent;border:none;color:#9ca3af;cursor:pointer;font-size:0.95rem;padding:4px 6px; }

/* transient popover */
.slider-province-card .transient-popover { position:absolute; z-index:9999; background:#0284c7; color:#fff; padding:6px 8px; border-radius:6px; font-size:0.85rem; box-shadow: 0 8px 30px rgba(2,6,23,0.45); pointer-events:none; transform: translateY(-6px); opacity:0; transition: opacity 180ms ease, transform 180ms ease; }
.slider-province-card .transient-popover.show { opacity:1; transform: translateY(0); }

/* replicate confirm popover */
.slider-province-card .replicate-popover { position:absolute; z-index:10000; width:220px; background: linear-gradient(180deg,#0b1220, #0f1724); color:#e6eef1; padding:10px; border-radius:8px; font-size:0.92rem; box-shadow: 0 12px 40px rgba(2,6,23,0.6); pointer-events:auto; opacity:0; transform: translateY(-6px) scale(0.98); transition: opacity 160ms ease, transform 160ms ease; }
.slider-province-card .replicate-popover.show { opacity:1; transform: translateY(0) scale(1); }
.slider-province-card .replicate-popover .rp-msg { font-weight:600; margin-bottom:8px; color:#f8fafc; }
.slider-province-card .replicate-popover .rp-actions { display:flex; gap:8px; justify-content:flex-end; }
...
.slider-province-card .st-list { color:#000; }
.slider-province-card .st-item { padding:8px; border-radius:6px; cursor:pointer; color:#000; font-size:0.92rem; border:1px solid rgba(2,6,23,0.04); background:transparent; }
.slider-province-card .st-item:hover { background: #f8fafc; }

/* ---------- Thicker borders for card-like UI elements ---------- */
...
.rsm-st-row,
.st-item {
  border: 3px solid rgba(2,6,23,0.12) !important; /* medium gray — 3px thick */
  border-radius: 10px !important;
  box-sizing: border-box;
}

/* slightly stronger highlight when hovering a bordered card */
.container-cards .card:hover,
.rsm-provinces-list .province-sublist .city-item:hover,
.slider-province-card .province-item:hover,
.slider-province-card .st-item:hover {
...
/* Province list & Total‑STs are displayed inside the right-side panel; visibility is controlled with `aria-hidden` on each element (do not globally hide them here). */

/* visible wrapper shown next to the image — top anchored; expand downward only */
.slider-province-wrap {
    ...
    top: 12px;                /* anchor top — DO NOT move */
    right: -950px;              /* keep close/overlapping the image */
    bottom: auto;            /* allow height to control bottom */
...
/* position province list left within the region panel (no markup changes) */
#sliderProvinceWrap .province-panel-body { display:flex; gap:12px; align-items:flex-start; }
